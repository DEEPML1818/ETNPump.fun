...
async function main() {
  ...
  // The factory uses a default max sell amount (DEFAULT_MAX_SELL_AMOUNT) as defined in the contract.
  ...
  for (const tokenDef of tokenDefinitions) {
    ...
    // (Note: this simple version does not include social media fields aside from description and image URL.)
    ...
    // First, check user1's token balance (using the PumpFunToken interface).
    // First, check user1's token balance (using the PumpFunToken interface).
    // First, check user1's token balance (using the PumpFunToken interface).
    ...
    // Administrative Updates: Update max sell amount (double it), pause and resume sells.
    const newMaxSell = (await router.maxSellAmount()) * 2n;
    console.log("Current max sell amount:", (await router.maxSellAmount()).toString());
    console.log("Current max sell amount (after doubling):", newMaxSell.toString());
    console.log("Updating max sell amount to:", newMaxSell.toString());
    const txUpdate = await router.connect(deployer).updateMaxSellAmount(newMaxSell);
    await txUpdate.wait();

    console.log("Pausing sell operations...");
    await (await router.connect(deployer).setSellPaused(true)).wait();
    try {
      console.log("User1 attempting a sell while paused (should revert)...");
      await (await router.connect(user1).batchSell([ethers.BigNumber.from("1")])).wait();
      console.log("Unexpected success while paused!");
    } catch (error) {
      console.log("Sell operation reverted as expected while paused.");
    }
    console.log("Resuming sell operations...");
    await (await router.connect(deployer).setSellPaused(false)).wait();
    ...
    for (let m = 0; m < 5; m++) {
      ...
      if (user2TokenBalance.gt(0)) {
        const sellAmountUser2 = user2TokenBalance.div(3);
